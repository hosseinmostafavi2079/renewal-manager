...
class Customer(models.Model):
    ...
    # فیلد جدید برای مسدودسازی ارسال پیامک به یک مشتری خاص
...
class ServiceItem(models.Model):
    SERVICE_TYPES = (
        ('domain', 'ثبت/تمدید دامنه'),
        ('server', 'سرور (اختصاصی/ابری)'),
        ('ssl', 'گواهینامه SSL'),
    )
    PAYMENT_STATUS = (
        ('pending', 'در انتظار تایید پرداخت'),
        ('paid', 'پرداخت شده / تسویه'),
    )
...
# جدول جدید: ثبت تاریخچه پرداخت‌ها و تمدیدهای انجام شده
...
class AlertHistory(models.Model):
    ALERT_TYPES = (
        ('sms', 'ارسال پیامکی (Kavenegar)'),
        ('bale', 'پیام‌رسان بله (Bale)'),
    )
